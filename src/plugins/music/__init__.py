import datetime
import typing
from utils.nonebot_uils import commandHandle
from utils import aiorequests
from bot import logger
from nonebot.adapters.cqhttp import Bot, Event,MessageSegment



# from aiocqhttp.message import MessageSegment

# from hoshino import Service

from .search_netease_cloud_music import search as search163
from .search_qq_music import search as searchqq
from .search_migu_music import search as searchmigu

MANUAL = '''
[点歌 好日子] 混合搜索
[搜网易云 好日子] 搜索网易云
[搜QQ音乐 好日子] 搜索QQ音乐
[搜咪咕音乐 好日子] 搜索咪咕音乐
'''.strip()

cool_down = datetime.timedelta(minutes=3)  # 冷却时间
expire = datetime.timedelta(minutes=2)

temp: typing.Dict[str, typing.Any] = {}
last_check: typing.Dict[str, datetime.datetime] = {}


@commandHandle('选',aliases={'选', '选择', '选歌'})
async def choose_song(bot: Bot, event: Event, state: dict):
    key = f'{event.group_id}-{event.user_id}'
    if key not in temp:
        if str(event.group_id) in last_check and datetime.datetime.now() - last_check[str(event.group_id)] < expire:
            await bot.send(event, '不可以替他人选歌哦', at_sender=True)
        return
    song_dict = temp[key]
    song_idx = []
    for msg_seg in event.message:
        if msg_seg.type == 'text' and msg_seg.data['text']:
            song_idx.append(msg_seg.data['text'].strip())
    if not song_idx:
        await bot.send(event, '你想听什么呀?', at_sender=True)
    else:
        song_idx = ''.join(song_idx)
        for idx in song_idx:
            if idx in song_dict:
                song = song_dict[idx]
                if song['type'] == '163':
                    music = MessageSegment.music(song['type'], song['id'])
                elif song['type'] == 'custom':
                    music = MessageSegment(type_='music', data=song)
                else:
                    music = MessageSegment(
                        type_='music',
                        data={
                            'id': str(song['id']),
                            'type': song['type'],
                            'content': song['artists']
                        }
                    )
                await bot.send(event, music)
        del temp[key]
        del last_check[str(event.group_id)]
        # else:
        #     await bot.send(event, '只能选择列表中有的歌曲哦', at_sender=True)


@commandHandle('点歌', aliases={'搜歌曲', '我想听'})
async def to_apply_for_title(bot: Bot, event: Event, state: dict):
    if str(event.user_id) in last_check:
        intervals = datetime.datetime.now() - last_check[str(event.user_id)]
        if intervals < cool_down:
            await bot.send(event, f'人家很忙，请{(cool_down - intervals).seconds}秒之后再点歌哦~')
            return
    music_name = []
    for msg_seg in event.message:
        if msg_seg.type == 'text' and msg_seg.data['text']:
            music_name.append(msg_seg.data['text'].strip())
    if not music_name:
        await bot.send(event, '你想听什么呀?', at_sender=True)
    else:
        music_name = ''.join(music_name)
        song_list = await search_music(music_name)
        if song_list:
            logger.info('成功获取到歌曲列表')
            key = f'{event.group_id}-{event.user_id}'
            temp[key] = {}
            # _music = MessageSegment.music(type_=_type, id_=_id)
            msg = ['我找到了这些~!']
            flag, flag2 = True, True
            if song_list[0]['type'] == '163':
                msg.append('===== NetEase')
            for idx, song in enumerate(song_list):
                if song['type'] == 'qq' and flag:
                    msg.append('===== QQ')
                    flag = False
                if song['type'] == 'custom' and song['subtype'] == 'migu' and flag2:
                    msg.append('===== Migu')
                    flag2 = False
                msg.append(
                    f'{idx}.{song["name"]}-{song["artists"]}'
                )
                temp[key][str(idx)] = song
            msg.append('')
            msg.append('发送[选择]+序号来听歌吧~')
            logger.info(msg)
            await bot.send(event, '\n'.join(msg))
            last_check[str(event.group_id)] = datetime.datetime.now()
            last_check[str(event.user_id)] = datetime.datetime.now()
        else:
            await bot.send(event, '什么也没有找到的说OxO')


@commandHandle('搜163',aliases= {'搜网易云'})
async def search_netease_cloud_music(bot: Bot, event: Event, state: dict):
    if str(event.user_id) in last_check:
        intervals = datetime.datetime.now() - last_check[str(event.user_id)]
        if intervals < cool_down:
            await bot.send(event, f'人家很忙，请{(cool_down - intervals).seconds}秒之后再点歌哦~')
            return
    music_name = []
    for msg_seg in event.message:
        if msg_seg.type == 'text' and msg_seg.data['text']:
            music_name.append(msg_seg.data['text'].strip())
    if not music_name:
        await bot.send(event, '你想听什么呀?', at_sender=True)
    else:
        music_name = ''.join(music_name)
        song_list = await search163(music_name, result_num=5)
        if song_list:
            logger.info('成功获取到歌曲列表')
            key = f'{event.group_id}-{event.user_id}'
            temp[key] = {}
            # _music = MessageSegment.music(type_=_type, id_=_id)
            msg = ['我找到了这些~!']
            for idx, song in enumerate(song_list):
                msg.append(
                    f'{idx}. {song["name"]} - {song["artists"]}'
                )
                temp[key][str(idx)] = song
            msg.append('')
            msg.append('发送[选择]+序号来听歌吧~')
            await bot.send(event, '\n'.join(msg), at_sender=True)
            last_check[str(event.group_id)] = datetime.datetime.now()
            last_check[str(event.user_id)] = datetime.datetime.now()
        else:
            await bot.send(event, '什么也没有找到的说OxO')


@commandHandle('搜qq音乐', aliases={'搜QQ音乐'})
async def search_qq_music(bot: Bot, event: Event, state: dict):
    if str(event.user_id) in last_check:
        intervals = datetime.datetime.now() - last_check[str(event.user_id)]
        if intervals < cool_down:
            await bot.send(event, f'人家很忙，请{(cool_down - intervals).seconds}秒之后再点歌哦~')
            return
    music_name = []
    for msg_seg in event.message:
        if msg_seg.type == 'text' and msg_seg.data['text']:
            music_name.append(msg_seg.data['text'].strip())
    if not music_name:
        await bot.send(event, '你想听什么呀?', at_sender=True)
    else:
        music_name = ''.join(music_name)
        song_list = await searchqq(music_name, result_num=5)
        if song_list:
            logger.info('成功获取到歌曲列表')
            key = f'{event.group_id}-{event.user_id}'
            temp[key] = {}
            # _music = MessageSegment.music(type_=_type, id_=_id)
            msg = ['我找到了这些~!']
            for idx, song in enumerate(song_list):
                msg.append(
                    f'{idx}. {song["name"]} - {song["artists"]}'
                )
                temp[key][str(idx)] = song
            msg.append('')
            msg.append('发送[选择]+序号来听歌吧~')
            await bot.send(event, '\n'.join(msg), at_sender=True)
            last_check[str(event.group_id)] = datetime.datetime.now()
            last_check[str(event.user_id)] = datetime.datetime.now()
        else:
            await bot.send(event, '什么也没有找到的说OxO')


@commandHandle('搜migu', aliases={'搜咪咕音乐'})
async def search_migu_music(bot: Bot, event: Event, state: dict):
    if str(event.user_id) in last_check:
        intervals = datetime.datetime.now() - last_check[str(event.user_id)]
        if intervals < cool_down:
            await bot.send(event, f'人家很忙，请{(cool_down - intervals).seconds}秒之后再点歌哦~')
            return
    music_name = []
    for msg_seg in event.message:
        if msg_seg.type == 'text' and msg_seg.data['text']:
            music_name.append(msg_seg.data['text'].strip())
    if not music_name:
        await bot.send(event, '你想听什么呀?', at_sender=True)
    else:
        music_name = ''.join(music_name)
        song_list = await searchmigu(music_name, result_num=5)
        if song_list:
            logger.info('成功获取到歌曲列表')
            key = f'{event.group_id}-{event.user_id}'
            temp[key] = {}
            # _music = MessageSegment.music(type_=_type, id_=_id)
            msg = ['我找到了这些~!']
            for idx, song in enumerate(song_list):
                msg.append(
                    f'{idx}. {song["title"]} - {song["content"]}'
                )
                temp[key][str(idx)] = song
            msg.append('')
            msg.append('发送[选择]+序号来听歌吧~')
            await bot.send(event, '\n'.join(msg), at_sender=True)
            last_check[str(event.group_id)] = datetime.datetime.now()
            last_check[str(event.user_id)] = datetime.datetime.now()
        else:
            await bot.send(event, '什么也没有找到的说OxO')


async def search_music(music_name: str) -> typing.Union[list, dict]:
    result = []
    song_list = await search163(music_name, 3)
    if song_list:
        result += song_list
    # song_list = await searchqq(music_name, 3)
    # if song_list:
    #     result += song_list
    # song_list = await searchmigu(music_name, 3)
    # if song_list:
    #     result += song_list
    return result
